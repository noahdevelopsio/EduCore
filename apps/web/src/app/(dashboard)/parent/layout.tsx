...
import { LayoutDashboard, LogOut, ClipboardCheck, PenTool, Megaphone, CreditCard } from 'lucide-react';

const MENUS = [
    { name: 'Dashboard', href: '/parent', icon: LayoutDashboard },
    { name: 'Attendance', href: '/parent/attendance', icon: ClipboardCheck },
    { name: 'Results', href: '/parent/results', icon: PenTool },
        ...
        <div className="min-h-screen bg-neutral-50 flex">
            ...
            <aside className="w-64 bg-white border-r border-neutral-200 flex flex-col hidden md:flex fixed h-full z-10">
                <div className="h-16 flex items-center px-6 border-b border-neutral-200">
                    <span className="font-display font-bold text-xl text-primary-600">EduCore Parent</span>
                </div>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    {MENUS.map((menu) => {
                        ...
                        if (menu.href === '/parent' && pathname !== '/parent') return (
                            <Link
                                key={menu.href}
                                href={menu.href}
